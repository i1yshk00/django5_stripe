...
# В production публичные хосты задаются явно через переменные окружения.
#
# `127.0.0.1` и `localhost` всегда добавляются дополнительно — это нужно для:
# 1. docker compose healthcheck'а, который изнутри контейнера бьет по
#    `http://127.0.0.1:8000/health/` и без этого получает 400 DisallowedHost;
# 2. CD-скрипта, который проверяет `/health/` curl-ом изнутри сервера;
# 3. локальных диагностических вызовов через `docker exec`.
#
# Безопасно: даже если злоумышленник подделает Host: 127.0.0.1, физически
# обратиться к серверу с loopback-адреса можно только из самой машины.
